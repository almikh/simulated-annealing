...
void MainWindow::createToolbar() {
  ...
  auto run = new QPushButton("Run task", toolbar);
  label->setFixedWidth(64);
  run->setFixedWidth(128);

  max_cities_->setFixedWidth(128);
  max_cities_->setValidator(new QIntValidator(2, 1000000, this));
  max_cities_->setText(QString::number(100));
...
void MainWindow::timerEvent(QTimerEvent* event) {
  ...
  float sy = pixmap.height() * 1.0f / (s + 1.0f);
  float sx = pixmap.width() * 1.0f / (s + 1.0f);
  float dx = sx * 0.5f, dy = sy * 0.5f;

  painter.setPen(Qt::blue);
  for (auto& e : state_) {
    auto p = coords_[e];
    painter.drawEllipse(dx + p.x() * sx - 2, dy + p.y() * sy - 2, 4, 4);
  }
